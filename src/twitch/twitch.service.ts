...
export class TwitchService {
  ...
  async getSubs(userId: string, accessToken: string): Promise<AxiosResponse> {
    const headers = {
  ...
  async getViews(userId: string, accessToken: string): Promise<any> {
    ...
    subs.push(...followsData.data); // save views
    cursor = followsData.pagination.cursor; // save last page

    } while (cursor);

    // Get Views ID
    const userIds: any = subs.map((follow) => follow.from_id);
    ...
    console.log("VIEWS TOTAL", subs.length);
